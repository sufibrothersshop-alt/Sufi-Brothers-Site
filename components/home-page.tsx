...
import { branchName, branchPhone, branchPhoneDisplay, type Branch } from '@/lib/branches'
...
export function HomePage({
  ...
  branch: Branch
  initialMenuItems: ResolvedMenuItem[]
  initialCategoryIcons?: Record<string, string>
}) {
  const menuItems = useResolvedMenu(branch, initialMenuItems)
  const categoryIcons = useCategoryIcons(branch, initialCategoryIcons)
  const deliveryEnabled = useDeliveryStatus(branch)
  // Categories come from whatever this branch's items actually use, not a
  // fixed list — an admin typing a new category name into "Add item" makes
  // it show up here automatically.
  const menuCategories = useMemo(() => categoriesFromItems(menuItems), [menuItems])
  ...
  const filteredDishes = useMemo(() => menuItems.filter((dish) => {
    const categoryMatch = dish.category === currentCategory
    ...
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="home" className="relative mx-auto grid max-w-7xl items-center gap-10 px-5 pb-16 pt-5 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:pb-20 lg:pt-8">
        ...
        <div className="relative min-h-[360px] sm:min-h-[460px]">
          ...
          <img src="/Burgers/zinger-burger.webp" alt="Sufi Brothers zinger burger" className="relative z-0 h-[390px] w-full rounded-[42%_42%_12%_12%] object-cover shadow-2xl shadow-primary/20 sm:h-[500px]" />
