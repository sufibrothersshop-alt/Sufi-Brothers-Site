...
import { Bike, CheckCircle2, Clock3, Flame, MessageCircle, Phone, X, XCircle } from 'lucide-react'
...
export function OrderTrackerWidget({
  ...
  riderPhone,
  onDismiss,
}: OrderTrackerWidgetProps) {
  const [expanded, setExpanded] = useState(false)
  const PhaseIcon = isCancelled ? XCircle : isDelivered ? CheckCircle2 : PHASE_ICONS[currentPhaseIndex]
  const phaseLabel = isCancelled ? 'Order cancelled' : isDelivered ? 'Delivered' : ORDER_PHASES[currentPhaseIndex].label
  ...
  return (
    <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-3">
        ...
        <div className="w-72 rounded-3xl border border-border bg-card p-5 shadow-2xl">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className={`mt-0.5 font-serif text-lg font-black ${isCancelled ? 'text-destructive' : ''}`}>{phaseLabel}</p>
            </div>
          ...
          {isCancelled ? (
            <p className="mt-4 text-sm text-muted-foreground">This order was cancelled. Call us if that doesn't seem right.</p>
          ) : (
            <>
              ...
              {riderName && riderPhone && (
                <div className="mt-4 flex items-center justify-between gap-2 rounded-xl bg-secondary/20 px-3 py-2.5">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Your rider</p>
                    <p className="text-sm font-bold">{riderName}</p>
                  </div>
                  <div className="flex gap-1.5">
                    <a href={`tel:${riderPhone}`} aria-label={`Call ${riderName}`} className="flex size-8 items-center justify-center rounded-full bg-card text-primary shadow">
                      <Phone className="size-4" />
                    </a>
                    <a
                      href={buildWhatsAppLink(riderPhone, `Hi ${riderName}, this is regarding my Sufi Brothers order #${order.id.slice(0, 8)}.`)}
                      target="_blank"
                      rel="noreferrer"
                      aria-label={`WhatsApp ${riderName}`}
                      className="flex size-8 items-center justify-center rounded-full bg-card text-primary shadow"
                    >
      ...
      >
        {!isCancelled && <OrderTrackerRing progress={progress} size={64} strokeWidth={6} />}
        <PhaseIcon className={`absolute size-5 ${isCancelled ? 'text-destructive' : 'text-primary'}`} />
      </button>
    </div>
  )
}
